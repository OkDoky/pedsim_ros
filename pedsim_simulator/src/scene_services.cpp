...
bool SceneServices::spawnPeds(pedsim_srvs::SpawnPeds::Request &request, pedsim_srvs::SpawnPeds::Response &response)
{
  ...
  for (int ped_i = 0; ped_i < (int)request.peds.size(); ped_i++)
  {
    ...
    std::vector<flatland_msgs::Model> new_models = getFlatlandModels(ped, new_agent_ids, ns);
...
std::vector<flatland_msgs::Model> SceneServices::getFlatlandModels(pedsim_msgs::Ped ped, std::vector<int> ids, string ns)
{
...
//parksangback
    // model.name = ns +"/person_" + std::to_string(ids[i]);
